// ============================================
// Ashen Citadel - Core Game Prototype v0.4
// Added: Hero Level Up + Star Upgrade
// ============================================
...
const Game = {
  ...
  heroes: [
    { id: "kaveh", name: "کاوه آهنین", role: "Vanguard", roleFa: "محافظ", level: 1, stars: 1, hp: 420, atk: 35, def: 55, skill: "سپر فولادی", owned: true, description: "تانک خط مقدم" },
    { id: "arash", name: "آرش آتشین", role: "Destroyer", roleFa: "نابودگر", level: 1, stars: 1, hp: 280, atk: 78, def: 22, skill: "تیر آتشین", owned: true, description: "تیرانداز" },
    { id: "anahita", name: "آناهیتا", role: "Support", roleFa: "پشتیبان", level: 1, stars: 1, hp: 310, atk: 28, def: 30, skill: "چشمه حیات", owned: false, description: "درمانگر" },
    { id: "rostam", name: "رستم سایه", role: "Destroyer", roleFa: "نابودگر", level: 1, stars: 2, hp: 300, atk: 95, def: 28, skill: "ضربه سایه‌ای", owned: false, description: "قاتل" }
  ],
  selectedBuilding: null, inDefense: false, gameStarted: false,
  defenseWave: 1, defenseEnemies: [], defenseHeroes: [], defenseRunning: false, _loadedFromSave: false,
  ...
  startGame() {
    ...
    this.notify(this._loadedFromSave ? 'خوش برگشتی، فرمانده!' : 'به دژ خاکستر خوش آمدی، فرمانده!');
    if (!this._loadedFromSave) setTimeout(() => this.notify('دژ را گرم نگه دار و مردم را نجات بده.'), 1200);
    this.saveGame(true);
  },
  ...
  saveGame(silent = false) {
    try {
      const data = {
        version: 4,
        resources: this.resources, heat: this.heat, heatMax: this.heatMax,
        heatProduction: this.heatProduction, heatConsumption: this.heatConsumption,
        buildings: {}, production: this.production,
      ...
      for (let key in this.buildings) data.buildings[key] = { level: this.buildings[key].level };
  ...
  loadGame() {
    try {
      ...
      if (data.buildings) for (let key in data.buildings) if (this.buildings[key]) this.buildings[key].level = data.buildings[key].level;
  ...
  drawBase() {
    ...
    for (let i = 0; i < 12; i++) {
      ctx.fillRect((Date.now()/40 + i*37) % w, (Date.now()/25 + i*53) % (h-80), 2, 2);
    }
    ...
    if (this.heat > 15) {
      ...
      ctx.fillStyle = `rgba(255,110,30,${0.12+this.heat/450})`; ctx.fill();
    }
  ...
  getHeroLevelCost(hero) {
    const lv = hero.level;
    return { food: Math.floor(40*Math.pow(1.35,lv-1)), iron: Math.floor(8*Math.pow(1.4,lv-1)), crystal: lv>=5 ? Math.floor(1+(lv-4)*0.5) : 0 };
  },
  getHeroStarCost(hero) {
    const s = hero.stars;
    return { crystal: 5+s*4, iron: 30*s, food: 80*s };
  },
  ...
  renderHeroList() {
    ...
    this.heroes.forEach(h => {
      ...
      if (!h.owned) {
        ...
        const lvCost = this.getHeroLevelCost(h), starCost = this.getHeroStarCost(h);
        let lvT = 'غذا '+lvCost.food; if (lvCost.iron) lvT += ' • آهن '+lvCost.iron; if (lvCost.crystal) lvT += ' • 💎 '+lvCost.crystal;
        let stT = '💎 '+starCost.crystal+' • آهن '+starCost.iron+' • غذا '+starCost.food;
  ...
  levelUpHero(id) {
    const hero = this.heroes.find(h => h.id===id);
    if (!hero || !hero.owned || hero.level>=20) return;
    const cost = this.getHeroLevelCost(hero);
    if (this.resources.food<cost.food || this.resources.iron<cost.iron || this.resources.crystal<(cost.crystal||0)) { this.notify('منابع کافی برای ارتقا نیست!'); return; }
    this.resources.food -= cost.food; this.resources.iron -= cost.iron;
    if (cost.crystal) this.resources.crystal -= cost.crystal;
    hero.level++;
    const g = 1 + hero.stars*0.03;
    hero.hp = Math.floor(hero.hp*(1.08+g*0.02));
    hero.atk = Math.floor(hero.atk*(1.07+g*0.02));
    hero.def = Math.floor(hero.def*(1.06+g*0.02));
    this.notify(hero.name+' به سطح '+hero.level+' رسید!');
    this.updateUI(); this.renderHeroList(); this.saveGame(true);
  },

  starUpHero(id) {
    const hero = this.heroes.find(h => h.id===id);
    if (!hero || !hero.owned || hero.stars>=6) return;
    const cost = this.getHeroStarCost(hero);
    if (this.resources.crystal<cost.crystal || this.resources.iron<cost.iron || this.resources.food<cost.food) { this.notify('منابع کافی برای ستاره نیست!'); return; }
    this.resources.crystal -= cost.crystal; this.resources.iron -= cost.iron; this.resources.food -= cost.food;
    hero.stars++;
    hero.hp = Math.floor(hero.hp*1.18); hero.atk = Math.floor(hero.atk*1.15); hero.def = Math.floor(hero.def*1.12);
    this.notify(hero.name+' به '+hero.stars+' ستاره رسید! ⭐');
    this.updateUI(); this.renderHeroList(); this.saveGame(true);
  },
  ...
  startDefenseWave() {
    ...
    for (let i=0;i<count;i++) this.defenseEnemies.push({ x:40+Math.random()*280, y:-20-i*35, hp:60+this.defenseWave*25, maxHp:60+this.defenseWave*25, speed:0.6+Math.random()*0.4 });
  ...
  defenseLoop() {
    ...
    this.defenseEnemies.forEach(e => {
      ...
      this.defenseHeroes.forEach(hero => {
        if (hero.currentHp>0 && Math.abs(e.x-hero.x)<40 && e.y>hero.y-30) { hero.currentHp-=0.4; e.hp-=hero.atk*0.04; }
      });
    ...
    if (!heroesAlive || this.defenseEnemies.some(e=>e.y>360)) {
      ...
      this.notify('دژ آسیب دید... گرما و غذا کاهش یافت.'); this.updateUI(); this.saveGame(true);
      document.getElementById('btn-start-defense').style.display='inline-block'; return;
    }
